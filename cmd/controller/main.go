...
func main() {
	...
	// Watcher for the metrics server certificate. Squall serves no webhooks:
	// nothing calls SetupWebhookWithManager, so the manager never registers a
	// webhook server and none is configured here.
	var metricsCertWatcher *certwatcher.CertWatcher

	// The Metrics options configure the metrics server. More info:
	...
	if secureMetrics {
		...
		// can access the metrics endpoint. The RBAC lives in the Helm chart's controller-rbac.yaml. More info:
	...
	// Point -metrics-cert-path at a cert-manager-issued certificate to serve
	// the metrics endpoint with a trusted one instead.
	...
	idleMetrics := metrics.NewIdleCollector(internalclock.RealClock{})
	operationalMetrics := metrics.NewModelOperationalCollector()
	ctrlmetrics.Registry.MustRegister(ageMetrics, priceMetrics, uncontrolledMetrics, idleMetrics, operationalMetrics)
