...
type ModelSpec struct {
	...
	//
	// DO NOT RELY ON THIS YET (D161). It is wired correctly — the value
	// reaches job_spec.max_duration and the runner's submit body — but a
	// live test on the Kubernetes backend saw a replica run 104 minutes
	// against max_duration: 3600 without being terminated, with the
	// runner's own API still reporting state "running" and an empty
	// termination_reason. Treat it as unproven, not as the bound that
	// survives a dead controller, until it has been observed to fire.
	//
