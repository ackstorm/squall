...
type ModelSpec struct {
	...
	// the other.
	//
	// PIN THE INDEX DIGEST, NOT THE PER-ARCHITECTURE ONE (D162). A digest
	// may address either a multi-arch image index or a single leaf
	// manifest, and this pattern cannot tell them apart. Pin a leaf and
	// dstack derives that architecture as a hard requirement: an arm64
	// leaf on Vast.ai matched zero offers, and the failure surfaced as
	// `failed_to_start_due_to_no_capacity` naming a fleet on another
	// backend entirely — a message that sends you tuning CPU, memory and
	// price, none of which can fix it. The same repo's index digest
	// matched 38 offers. Get the right one with
	// `docker buildx imagetools inspect <repo>:<tag>` and use its
	// top-level `Digest:`; `docker pull` on an arm64 machine gives you the
	// leaf.
	...
	// one.
	//
	// Which mode it applies to, precisely: only the SLEEP flip is
	// on-demand-only — it is gated on MinReplicas == 0 (sleepDue, phase.go)
	// and does nothing to a pinned Model. The field is still read in BOTH
	// modes, ungated, as the staleness bound on readiness evidence (b)
	// (freshSuccess, model_controller.go) and as squall-proxy's
	// demand-refresh cadence. So it is NOT a no-op under MinReplicas: 1,
	// and it is deliberately not nested under an on-demand-only block that
	// would claim otherwise (ledger D167(b)).
	//
	...
	// left silently unwakeable. An IdleTimeout shorter than the controller's own idle
	...
	// truncation. Keep this comfortably above the requeue interval; the 15m
	// default is 60x it. Measured, ledger D171: against a 2s requeue,
	...
	//
	// IGNORED when MinReplicas: 1. The trigger is gated on MinReplicas == 0
	// (uncontrolledDue, phase.go): a pinned Model is meant to stay up
	// whether or not squall can currently see idle evidence, so there is
	// nothing here for this bound to protect against. Validation still
	// checks the value's range in both modes, so a pinned Model with a
	// nonsensical one is refused rather than silently accepted.
	// +optional
	...
	//
	// On-demand-only is literal: under MinReplicas: 1 the submission carries
	// NO max_duration at all (model_controller.go gates the assignment on
	// MinReplicas == 0), so this field is ignored for a pinned Model however
	// it is set. Contrast MaxLifetime above, which stays meaningful in both
	// modes precisely because it never actuates.
	//
	// DOES NOT FIRE ON THE KUBERNETES BACKEND (D161). It is wired
	// correctly — the value reaches job_spec.max_duration and the runner's
	// submit body — but it is not enforced. A live Model ran 104 minutes
	// against max_duration: 3600, and a minimal reproduction (a plain
	// nginx service, no squall involved, max_duration: 2m) was still
	// "running" at 621 seconds — 4.96x its own limit — with an empty
	// termination_reason, ending only when a human aborted it. Do not
	// treat this as the bound that survives a dead controller. Whether a
	// dockerized VM backend enforces it is untested.
	//
	// It also bounds ONE CONTINUOUS job submission, not the Model's
	// cumulative awake time: every sleep/wake cycle mints a new submission
	// with a fresh clock, so a Model that flips regularly never approaches
	// it. That is right for the runaway it targets and wrong as a spend cap.
